...
def compute_3D_bbs_from_gt_liver(config):
    ...
    masks_folders = list(filter(lambda x: x != ".DS_Store", masks_folders))
    ...
    for i in range(len(masks_folders)):
        ...
        for j in range(0, depth_of_volume):
            ...
            if is_liver:
                ...
                # results png
                if os.path.exists(os.path.join(liver_results, dir_name, png)): # Temporary fix until Mirriam replies
