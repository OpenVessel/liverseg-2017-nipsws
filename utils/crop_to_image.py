...
def crop(base_root, input_config='lesion/',
    ...
    for i in range(len(crops_lines)):
            ...
            if int(id_img.split(os.path.sep)[-1]) > 104:
                ...
                if bool_zoom == '1':
                    zoomed_mask = imageio.imread(os.path.join(input_results_path, id_img + '.png'))
                    #if zoomed_mask.shape[0] != 512:
                    print(input_results_path)
                    print(os.path.join(input_results_path, id_img + '.png'))
                    print("sf",mask.shape)
                    print(id_img)
                    #print(zoomed_mask)
                    print("zoom mask shaped",zoomed_mask.shape)
                    
                    # if zoomed_mask.shape[0] != 512:
                    #     zoomed_mask = zoomed_mask.reshape(512,512)

                    print(" min/max", id_img, bool_zoom, mina, maxa, minb, maxb )
                    #print(mask[mina:maxa, minb:maxb])
